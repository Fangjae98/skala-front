...
citySelect.addEventListener('change', async function(event) {
    ...
    try {
        ...
        weatherBox.innerHTML = `
            <div style="background-color: #e0e2e9; padding: 15px; border-radius: 6px; margin-top: 10px;">
                <h4> 🌍 ${cityName} 실시간 날씨 </h4>
                <p> <strong> 🌡️ 현재 기온:  ${currentTemp}°C </strong> </p>
                <p> <strong> 💧 현재 습도: ${currentHumidity}% </strong> </p>
            </div>
        `;

    } catch (error) {
        weatherBox.innerHTML = "<p> ⚠️ 데이터를 불러오지 못했습니다. </p>";
        console.error(error);
    }
});